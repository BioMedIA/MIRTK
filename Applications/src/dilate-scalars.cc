...
void PrintHelp(const char *name)
{
  ...
  cout << "\n";
  cout << "Description:\n";
  cout << "  Dilates scalar data of an input point set by replacing a value by the\n";
  cout << "  maximum of the adjacent data values. When the input data array has\n";
...
int main(int argc, char *argv[])
{
  ...
  if (process_pd) {
    ...
    if (verbose) cout << "Dilating point data with c=" << neighbors->Maximum() << "...", cout.flush();
    ...
    if (verbose) cout << "Dilating cell data...", cout.flush();
    DilateCellData filter;
    filter.Input(input);
    filter.DataName(array_name);
    filter.Iterations(iterations);
    filter.Run();
