...
void PrintHelp(const char *name)
{
  ...
  cout << "\n";
  cout << "Description:\n";
  cout << "  Closes scalar data of an input point set by perfoming a dilation\n";
  cout << "  followed by the same number of erosions. When the input data array\n";
...
int main(int argc, char *argv[])
{
  ...
  if (process_pd) {
    ...
    if (verbose) cout << "Closing point data with c=" << neighbors->Maximum() << "...", cout.flush();
    ...
    if (verbose) cout << "Closing cell data...", cout.flush();
    CloseCellData filter;
    filter.Input(input);
    filter.DataName(array_name);
    filter.Iterations(iterations);
    filter.Run();
