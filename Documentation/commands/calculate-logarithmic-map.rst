:orphan:

.. Auto-generated by mirtk-help-rst from "mirtk calculate-logarithmic-map -h" output

calculate-logarithmic-map
=========================

.. program:: calculate-logarithmic-map


Synopsis
--------

::

    mirtk calculate-logarithmic-map <disp> <velo> [options]
    mirtk calculate-logarithmic-map <dx> <dy> [<dz>] <vx> <vy> [<vz>] [options]


Description
-----------

.. include:: _descriptions/calculate-logarithmic-map.rst



Command options
---------------

.. option:: -ss

   Use scaling and squaring (SS) method for exponentiation. (default)

.. option:: -euler

   Use forward Euler method for exponentiation.

.. option:: -steps <int>

   Number of integration steps (2^n in case of SS). (default: 64)

.. option:: -terms <int>

   Number of BCH approximation terms (either 2 or 3). (default: 6)

.. option:: -iters <int>

   Number of BCH update steps. (default: 8)

.. option:: -smooth

   Smooth velocities before each update. (default: off)

.. option:: -jac

   Use Jacobian for Lie bracket computation. (default: off)


Standard options
----------------

.. option:: -v, -verbose [n]

   Increase/Set verbosity of output messages. (default: 0)

.. option:: -debug [level]

   Increase/Set debug level for output of intermediate results. (default: 0)

.. option:: -version [major.minor]

   Print version and exit or set version to emulate.

.. option:: -revision

   Print revision (or version) number only and exit.

.. option:: -h, -help

   Print help and exit.


Terminal options
----------------

.. option:: -color, -nocolor

   Enable/disable colored output. (default: off)


Parallelization options
-----------------------

.. option:: -threads <n>

   Use maximal <n> threads for parallel execution. (default: automatic)
