:orphan:

.. Auto-generated by mirtk-help-rst from "mirtk project-onto-surface -h" output

project-onto-surface
====================

.. program:: project-onto-surface


Synopsis
--------

::

    mirtk project-onto-surface <input> <output> -image <file> [options]
    mirtk project-onto-surface <input> <output> -labels <file> [options]
    mirtk project-onto-surface <input> <output> -boundary [-nolabel-points] [-nolabel-cells] [-name <scalars>]


Description
-----------

.. include:: _descriptions/project-onto-surface.rst



Arguments
---------

.. option:: input

   Input surface mesh.

.. option:: output

   Output surface mesh.


Command options
---------------

.. option:: -image <file>

   Input real-valued scalar/vector image.

.. option:: -labels <file>

   Input segmentation image with positive integer labels.

.. option:: -name <name>

   Name of output scalar array. (default: Scalars or Labels)

.. option:: -celldata, -nocelldata

   Assign values to cells of input surface. (default: off)

.. option:: -pointdata, -nopointdata

   Assign values to points of input surface. (default: on)

.. option:: -write-dilated-labels <file>

   Write image of dilated labels.

.. option:: -pial

   Input surface is cortical cGM/CSF boundary.

.. option:: -white

   Input surface is cortical WM/cGM boundary.

.. option:: -min-size <n>

   Surface patches with less than n points are removed. (default: 0)

.. option:: -fill, -nofill

   Fill holes in projected surface parcellation.


Standard options
----------------

.. option:: -v, -verbose [n]

   Increase/Set verbosity of output messages. (default: 0)

.. option:: -debug [level]

   Increase/Set debug level for output of intermediate results. (default: 0)

.. option:: -version [major.minor]

   Print version and exit or set version to emulate.

.. option:: -revision

   Print revision (or version) number only and exit.

.. option:: -h, -help

   Print help and exit.
