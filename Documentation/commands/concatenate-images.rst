:orphan:

.. Auto-generated by mirtk-help-rst from "mirtk concatenate-images -h" output

concatenate-images
==================

.. program:: concatenate-images


Synopsis
--------

::

    mirtk concatenate-images <input>... <output> [options]
    mirtk concatenate-images <input>... -output <output> [options]
    mirtk concatenate-images [options]


Description
-----------

.. include:: _descriptions/concatenate-images.rst



Command options
---------------

.. option:: -output <output>

   Output image file. Last positional argument if option missing.

.. option:: -image <input>

   Add one input image. Option can be given multiple times.

.. option:: -images <input>...

   Add one or more input images. Option can be given multiple times.

.. option:: -sort [on|off]

   Automatically determine correct order of input images
   based on the distance along the third or fourth image axis.

.. option:: -nosort

   Disable sorting of input images, same as :option:`-sort` off. (default)

.. option:: -start

   <float>     Position of first slice/frame of output volume/sequence.

.. option:: -spacing <float>

   Slice/Temporal spacing of output volume/sequence.


Standard options
----------------

.. option:: -v, -verbose [n]

   Increase/Set verbosity of output messages. (default: 0)

.. option:: -debug [level]

   Increase/Set debug level for output of intermediate results. (default: 0)

.. option:: -version [major.minor]

   Print version and exit or set version to emulate.

.. option:: -revision

   Print revision (or version) number only and exit.

.. option:: -h, -help

   Print help and exit.
