...
TEST(Downsampling, GaussianPyramid1)
{
  ...
  for (int l = 1; l < levels; ++l) {
    GaussianBlurring<irtkRealPixel> blur(.7 * pyramid[l-1].GetXSize(),
                                         .7 * pyramid[l-1].GetYSize(),
                                         .7 * pyramid[l-1].GetZSize());
    blur.SetInput (&pyramid[l-1]);
    blur.SetOutput(&pyramid[l]);
    blur.Run();
    Resampling<irtkRealPixel> resample(pyramid[l-1].GetX() / 2,
                                       pyramid[l-1].GetY() / 2,
                                       pyramid[l-1].GetZ() / 2);
    resample.SetInterpolator(InterpolateImageFunction::New(Interpolation_Linear, &pyramid[l]));
